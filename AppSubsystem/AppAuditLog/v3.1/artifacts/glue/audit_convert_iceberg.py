...
from pyspark.sql.functions import days, to_timestamp
...
class AuditLogTransformer:
    ...
    def run(self):
        ...
        df_audit = self.create_auditlog_df()
        ...
        self.logger.info(f"Loaded raw audit payloads. Count: {count}")

        self.write_to_s3(
            df=df_audit,
            table_name=self.args["TRANS_TABLE"],
            s3_location=f"s3://{self.args['S3_DST_BUCKET']}/{self.args['S3_DST_PREFIX']}",
    ...
    def create_auditlog_df(self) -> pyspark.sql.DataFrame:
        ...
        expected_cols = target_schema.fieldNames()
        ...
        columns_to_drop = [col for col in df.columns if col not in expected_cols]
        df = df.drop(*columns_to_drop)
        
        df = df.select(target_schema.fieldNames())
        
        return self.spark.createDataFrame(df.rdd, target_schema)

    def target_schema(self) -> StructType:
        # Get target table schema as a list of (name, type)
        table_name = self.args["TRANS_TABLE"]
        full_table = f"{self.args['CATALOG_NAME']}.{self.args['TRANS_DATALAKE']}.{table_name}"
        self.logger.info(f"target full table: {full_table}")
        return spark.table(full_table).schema


    def schema1(self) -> StructType:
        return StructType(
            [
                StructField("id", StringType(), False),
                StructField("correlationid", StringType(), True),
                StructField("startdatetime", StringType(), False),
                StructField("enddatetime", StringType(), False),                
                ...
                StructField("clientuser_type", StringType(), True),
                StructField("clienttime", StringType(), True),
                StructField("method", StringType(), True),
                StructField("headers", ArrayType(StringType()), False),
