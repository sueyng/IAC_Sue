# Backend ECS CloudFormation Templates - v5

This repository contains AWS CloudFormation templates for deploying ECS infrastructure and services.

## Key Features in v5

v5 introduces enhanced network security with ElastiCache dedicated subnets, environment-based database ports, and comprehensive resource tagging. The network configuration has been simplified for easier deployment and maintenance.
...
- Provides comprehensive IAM permissions for:
  ...
  - CloudWatch logs
  - SSM operations
  - Secrets Manager (scoped to `${AppShortName}-${EnvName}-*`)
  - X-Ray integration
...
{
  "ParameterKey": "PathPattern2",
  "ParameterValue": "/v2/api/*"
},
{
  "ParameterKey": "PathPattern3",
  "ParameterValue": "/legacy/*"
}
```
...
- ECS Service Security Group Egress:
  - HTTPS (443) to up to 3 VPC CIDR ranges
...
- ALB terminates SSL/TLS traffic with TLS 1.3 policy: `ELBSecurityPolicy-TLS13-1-2-2021-06`
...
- Single role used for both task execution and task runtime
- Secrets Manager access scoped to: `${AppShortName}-${EnvName}-*`
...
```json
{
  "ParameterKey": "ECRImage",
  "ParameterValue": "533267434948.dkr.ecr.ap-southeast-1.amazonaws.com/my-app:v5.2.1"
},
{
  "ParameterKey": "ADOTContainerImage",
  "ParameterValue": "533267434948.dkr.ecr.ap-southeast-1.amazonaws.com/utility-images/aws-observability/aws-otel-collector:v0.104.0"
}
```
...
```json
{
  "ParameterKey": "ServiceContainerPort",
  "ParameterValue": "80"
},
{
  "ParameterKey": "ContainerHealthCheckCommand", 
  "ParameterValue": "CMD-SHELL,curl -f http://localhost:80/health"
}
```
...
4. **Secret Access Errors**:
   ...
   - **Solution**:
     ...
     - Check secret follows naming pattern: `${AppShortName}-${EnvName}-*`
