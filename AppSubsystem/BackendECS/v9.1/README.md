# Backend ECS CloudFormation Templates - v9.1
...
- **ADOT ECS.20 Security Hub remediation**: ADOT collector task definition now runs the standard AWS ADOT collector image as non-root user `aoc`. No parameter override is introduced for v9.1.
...
> v9.1 runs the ADOT collector container as non-root user `aoc` to comply with ECS.20. This assumes the standard AWS ADOT collector image or a direct mirror that preserves the `aoc` runtime user.
