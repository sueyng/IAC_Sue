# AWS Lambda API Infrastructure (v1.2)
...
| v1.2 | **Checkmarx Remediation** — Four findings addressed: (1) **API Gateway Access Logging**: added optional access log setting to `AWS::ApiGateway::Deployment` via `StageDescription.AccessLogSetting`. New optional params `ApiGwAccessLogFormat` (default `""`) and `ApiGwLogRetentionInDays` (default `30`). When `ApiGwAccessLogFormat` is non-empty, a CloudWatch Log Group `/aws/apigateway/${APIName}-access-logs` is created and wired to the stage. (2) **Stack Notifications**: added optional `EnableStackNotifications` (yes/no, default `no`) and `StackNotificationSNSTopicArn` on both `cf-lambda-api-infra-main.yaml` and `cf-lambda-api-apps-main.yaml`. When enabled, `NotificationARNs` is set on every nested stack (Network+IAM, Authorizer, API Gateway, up to 20 Lambda apps). Recommended for NEW stack creation only — `NotificationARNs` on nested stacks is a create-time-only property. Parent-stack notifications must be passed via `--notification-arns` on `create-stack` / `update-stack` (AWS constraint). (3) **Lambda X-Ray Tracing**: added `TracingConfig.Mode: Active` to `LambdaAuthorizerFunction` in `cf-lambda-api-authorizer.yaml` (app Lambdas in `cf-lambda-api-app.yaml` already had it). Added scoped X-Ray write permissions (`xray:PutTraceSegments`, `xray:PutTelemetryRecords`) on `LambdaAuthorizerRole` (app Lambdas already inherit X-Ray perms via the shared execution role). (4) **API Gateway Response Compression**: added optional toggle `ApiGwEnableCompression` (yes/no, **default `no`** — safe for initial deployment) and `ApiGwMinimumCompressionSize` (default `10240` — 10 KB threshold, used only when toggle is `yes`). Recommended rollout: deploy with `no`, audit client gzip handling (esp. .NET HttpClient AutomaticDecompression), then switch to `yes` after testing. When enabled, responses ≥10 KB are gzipped; smaller payloads pass through. Application layer already supports compression; gateway compression is defense-in-depth. Backward compatible with v1/v1.1. IaCVersion tag: `AppSubsystem-BackendLambda-v1.2` |
| v1.2 post-release patch | Added optional `AuthorizerLambdaTimeout` parameter for the Lambda authorizer. Default `90` seconds supports longer authorizer token validation flows. |
...
### v1.2 — X-Ray VPC Interface Endpoint (required for trace delivery)

All Lambda functions in this stack (up to 20 app Lambdas + the authorizer) are **VPC-attached** and have `TracingConfig.Mode: Active` enabled. Because the Lambda security group uses CIDR-restricted egress (no `0.0.0.0/0`), the X-Ray daemon inside each Lambda cannot reach the public X-Ray API endpoint.

**Required:** the target VPC must have an X-Ray Interface VPC Endpoint:

- Service name: `com.amazonaws.<region>.xray` (e.g. `com.amazonaws.ap-southeast-1.xray`)
- Type: Interface
- Private DNS: enabled
- Attached to the same subnets as the Lambda `SubnetIds`
- Security group must allow HTTPS (443) inbound from the Lambda security group

**Without this endpoint:** Lambda functions still execute normally, but X-Ray trace segments will silently fail to upload — no traces will appear in the X-Ray console. Request the endpoint from the platform/network team before deployment if it does not already exist.
...
| Parameter Name | Type | Default | Mandatory | Sample Value | Description |
|---------------|------|----------|-----------|--------------|-------------|
| EnableLambdaAuthorizer | String | true | No | true | Enable or disable Lambda authorizer stack |
| AuthorizerCodeZipFileName | String | - | Yes | parent-child-stacks/Authorizer-1.0.0.zip | Lambda authorizer package S3 key |
| AuthorizerLambdaRuntime | String | dotnet8 | No | dotnet8 | Lambda authorizer runtime |
| AuthorizerLambdaHandler | String | - | Yes | Authorizer::Namespace.Function::FunctionHandler | Lambda authorizer handler |
| AuthorizerLambdaTimeout | Number | 90 | No | 90 | Lambda authorizer timeout in seconds, valid range 1-900 |
...
## v1.1 → v1.2 Upgrade Guide

v1.2 is **backward compatible** with v1.1. All new features default to OFF — re-using v1.1 parameter files on v1.2 works unchanged (missing keys fall back to safe defaults).

### Resource-by-Resource Impact on Stack Update

| Resource | Change | Impact |
|---|---|---|
| Lambda functions (20 app + 1 authorizer) | In-place update — `TracingConfig`, tags | Safe — no replacement, no downtime |
| `LambdaAuthorizerRole` | New inline policy (`LambdaAuthorizerXRayWriteAccessPolicy`) | Safe — in-place update |
| `ApiDeployment` | New optional `StageDescription.AccessLogSetting` | Only triggers when `ApiGwAccessLogFormat ≠ ""`. See note below. |
| `ApiGatewayAccessLogGroup` | New conditional resource | Only created when `ApiGwAccessLogFormat ≠ ""` |
| `PrivateApi` (REST API) | Tag update | Safe — tag only |
| All nested stacks | New `NotificationARNs` property | **No-op on stack update of existing nested stacks.** See critical note below. |

### ⚠️ Stack Notifications — Day-One New Provisioning ONLY

**`NotificationARNs` on `AWS::CloudFormation::Stack` is a CREATE-TIME-ONLY property.** The v1.2 template-level stack notification feature (`EnableStackNotifications` / `StackNotificationSNSTopicArn`) **only applies to brand-new stack creation**.

- **NEW stack (day one):** set `EnableStackNotifications: yes` and provide `StackNotificationSNSTopicArn` in parameters. All 23 nested stacks (Network+IAM, Authorizer, API Gateway, up to 20 Lambda apps) receive notifications automatically.
- **EXISTING stack (already deployed under v1 or v1.1):** these parameters will be **silently ignored for existing nested stacks on update**. Do not rely on the template to retrofit notifications.

**For existing stacks, the project team must enable notifications manually via the AWS Console** — per nested stack:

1. CloudFormation Console → select the nested stack (e.g. `*-ApiGatewayStack-*`, `*-LambdaApp1-*`, …)
2. Update → Use current template → Next
3. Next through Parameters → on "Configure stack options" scroll to **Notification options** → add the SNS topic ARN
4. Next → Submit

Repeat for every nested stack in use. The parent stack's own notification is configured at `create-stack` / `update-stack` time via the Console's "Notification options" field (AWS design constraint — the template cannot self-configure its own notification target).

### Note — API Gateway Access Logging on Existing Stacks

If enabling access logging on an already-deployed v1.1 stack, the `StageDescription.AccessLogSetting` change on the existing `ApiDeployment` may not propagate cleanly to the existing stage (known AWS behavior with `StageDescription`). If logs do not appear after update:
- Workaround A: manually enable access logging on the stage via API Gateway Console → Stages → Logs/Tracing tab. The template's next update will then align with it.
- Workaround B: rename the `ApiDeployment` logical ID to force a new deployment resource (causes brief API interruption during stage re-pointing).

### Note — X-Ray Trace Delivery

Lambda execution is unaffected by the X-Ray change, but traces will not reach the X-Ray console unless the VPC has the `com.amazonaws.<region>.xray` Interface Endpoint (see Prerequisites section).

### Pre-Upgrade Checklist
- [ ] Upload v1.2 templates to the project's S3 templates bucket (same filenames — overwrites v1.1)
- [ ] (Optional) Update parameter files with new keys, or rely on defaults:
  - `ApiGwAccessLogFormat` (default `""`), `ApiGwLogRetentionInDays` (default `30`)
  - `EnableStackNotifications` (default `"no"`), `StackNotificationSNSTopicArn` (default `""`)
- [ ] If enabling access logging: confirm log retention policy aligns with org standard
- [ ] If enabling stack notifications: SNS topic must already exist; subscribe intended recipients
- [ ] If tracing is desired: confirm X-Ray VPC Interface Endpoint exists
- [ ] Take CloudFormation drift report before upgrade as baseline
- [ ] **Stack notifications are day-one only.** If upgrading an existing stack and notifications are required, plan manual Console steps per nested stack (up to 23 stacks if all Lambda slots are used) — see section above.

### IaCVersion Tag
All resources updated from `AppSubsystem-BackendLambda-v1.1` to `AppSubsystem-BackendLambda-v1.2`
