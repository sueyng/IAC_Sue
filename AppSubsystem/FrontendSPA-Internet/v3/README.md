...
2. **Security Features**:
   ...
   - **NEW: AWS WAF Web ACL integration for application layer protection**
...
| **WAFWebACLArn** | **NEW: WAF Web ACL ARN for application protection** | **"arn:aws:wafv2:global:..."** | **No (leave empty to disable)** |
...
4. **IMPORTANT: For initial deployment, leave `WAFWebACLArn` parameter empty - WAF will be enabled later by HCC team**

## WAF Integration (New in v3)

The template now supports AWS WAF Web ACL integration for enhanced security:

### ⚠️ IMPORTANT: WAF Deployment Process

**For new CloudFront deployments, follow this process:**

1. **Initial Deployment**:
   - Set `WAFWebACLArn` parameter to empty string `""` 
   - Deploy the CloudFront stack first without WAF
   - This creates the CloudFront distribution without WAF association

2. **WAF Enablement Process**:
   - Contact HCC team to enable WAF for your CloudFront distribution
   - HCC team will manually enable WAF from AWS Console
   - Wait for HCC team confirmation that WAF is enabled

3. **Stack Update After WAF Enablement**:
   - Navigate to AWS WAF Console
   - Copy the WAF Web ACL ARN from the console
   - Update your parameter JSON file with the copied ARN
   - Execute CloudFormation stack update through your pipeline
   - This ensures subsequent stack updates maintain WAF integration

**Why this process is required:**
- Prevents WAF disassociation during future stack updates
- Maintains consistency between manual WAF enablement and CloudFormation state
- Ensures HCC team has control over WAF configuration before CloudFormation takes over
...
2. **WAF Requirements**:
   - WAF Web ACL must be created and managed by HCC team
   - Must be a CloudFront-compatible Web ACL (global scope)
   - ARN must be captured from AWS WAF Console after enablement

3. **Example WAF ARN Format**:
   ```
   arn:aws:wafv2:global:123456789012:webacl/MyWebACL/12345678-1234-1234-1234-123456789012
   ```
...
- **NEW: Optional WAF Web ACL integration for application layer protection**

### WAF Security (New in v3)
- Application layer DDoS protection
...
1. **Regional Requirements**:
   ...
   - **NEW: WAF Web ACLs must be global scope for CloudFront**
...
4. **Logging and Monitoring**:
   ...
   - **NEW: WAF logs available separately if WAF is enabled**
...
5. **Performance and Cost**:
   ...
   - **NEW: WAF adds minimal latency but provides significant security benefits**
...
7. **Compliance and Security**:
   ...
   - **NEW: Enhanced security with optional WAF protection**
...
## Version 3 Changes Summary

### New Features
- **WAF Integration**: Optional AWS WAF Web ACL association for enhanced security
- **Enhanced Security**: Application layer protection against common web threats
- **HCC-Managed WAF Process**: Structured workflow for WAF enablement through HCC team

### New Parameters
- `WAFWebACLArn`: Optional parameter for WAF Web ACL ARN (leave empty for initial deployment, update after HCC enables WAF)

### Enhanced Security
- WAF protection against OWASP Top 10 vulnerabilities
- Rate limiting and DDoS protection at application layer
- IP reputation and geo-blocking capabilities
- Custom security rules for application-specific threats

### Deployment Process Changes
- **Two-stage deployment**: Initial CloudFront deployment without WAF, followed by WAF enablement by HCC team
- **Manual-to-CloudFormation handoff**: Process to maintain WAF integration in subsequent stack updates
- **HCC team coordination**: Structured process for WAF enablement and management

### Backward Compatibility
- All v2 parameters remain unchanged
- Existing deployments can be upgraded by following the WAF enablement process
- WAF is completely optional and disabled by default for new deployments