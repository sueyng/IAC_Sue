# AWS Resources Naming Convention - Data Loading Template Version 5

This document outlines the standardized naming conventions for AWS resources deployed via the Data Loading CloudFormation template Version 5.

**Version 5 Changes:**
- Extended VPC CIDR support with VpcCidr4 and VpcCidr5 parameters
- Now supports up to 5 VPC CIDR ranges (previously 3) for enhanced multi-VPC connectivity
- Enhanced security group flexibility for complex network topologies
- Improved support for multi-VPC peering, hybrid cloud, and cross-region architectures
- Updated IaCVersion tag to AppSubsystem-BatchJob-v5

**Version 4 Changes:**
- Added customizable environment variable parameters matching ECS service pattern
...
- Updated IaCVersion tag to AppSubsystem-BatchJob-v4
...
- **IaCVersion**: `AppSubsystem-BatchJob-v5` (identifies resources created by this template version)

### Standard Tags Applied to All Resources
| Tag Key | Tag Value | Purpose |
|---------|-----------|---------|
| Name | Resource-specific naming pattern | Resource identification |
| IaCVersion | AppSubsystem-BatchJob-v5 | Version tracking and governance |
...
| AwsSecretPrefix | `${EnvVarConnectionStringSecretName}` | When EnvVarConnectionStringSecretName is provided |
| `${EnvVar1Name}` | `${EnvVar1Value}` | When EnvVar1Name is provided |
| `${EnvVar2Name}` | `${EnvVar2Value}` | When EnvVar2Name is provided |
| `${EnvVar3Name}` | `${EnvVar3Value}` | When EnvVar3Name is provided |

**Version 4 Changes:**
- **Added**: Customizable environment variables for application-specific configuration
- **Added**: `AwsSecretPrefix` environment variable for database connection string secrets
- **Added**: Up to 3 custom environment variable pairs (EnvVar1Name/Value, EnvVar2Name/Value, EnvVar3Name/Value)
- **Enhanced**: Conditional environment variable inclusion matching ECS service pattern
- **Improved**: Consistency with ECS service environment variable handling
...
aws resourcegroupstaggingapi get-resources --tag-filters Key=IaCVersion,Values=AppSubsystem-BatchJob-v5

# Filter resources by both version and environment
aws resourcegroupstaggingapi get-resources --tag-filters Key=IaCVersion,Values=AppSubsystem-BatchJob-v5 Key=Name,Values=*nprd-dev*
```

## Network Configuration Parameters (Version 5)

The template supports multiple VPC CIDR ranges for security group configuration:

| Parameter | Required | Description |
|-----------|----------|-------------|
| VpcCidr1 | Yes | Primary VPC CIDR IP Range |
| VpcCidr2 | No | Secondary VPC CIDR IP Range (optional) |
| VpcCidr3 | No | Tertiary VPC CIDR IP Range (optional) |
| VpcCidr4 | No | Quaternary VPC CIDR IP Range (optional) |
| VpcCidr5 | No | Quinary VPC CIDR IP Range (optional) |

**Use Cases for Multiple VPC CIDRs:**
- Multi-VPC peering architectures
- Hybrid cloud connectivity (on-premises via VPN/Direct Connect)
...
- **Backward Compatible**: No breaking changes - existing v4 deployments will continue to work
- **Optional**: Add VpcCidr4 and VpcCidr5 parameters to parameter files if additional VPC CIDR ranges are needed
- **Update parameter files** to include new optional VPC CIDR parameters (all default to empty)
- **Benefits**: Enhanced multi-VPC connectivity, support for complex network architectures

### From Version 3 to Version 4
- **Optional**: Add environment variable parameters to parameter files if custom environment variables are needed
- **Update parameter files** to include new optional environment variable parameters (all default to empty)
- **Application compatibility**: Existing applications will continue to work without changes
- **Benefits**: Enhanced environment variable configurability, consistency with ECS services, improved application configuration management
