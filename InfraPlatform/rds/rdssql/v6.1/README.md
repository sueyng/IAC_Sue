# Amazon RDS SQL Server Infrastructure Template v6.1
...
## What's New in v6.1
- **Event Subscription (db-instance)**: Added `maintenance` to `RdsInstanceEventSubscription` event categories.
- **Event Subscription (db-security-group)**: Added `failure` to `RdsSecurityGroupSubscription` event categories.
- **KMS Key Rotation**: Confirmed `EnableKeyRotation: true` remains set on both `RDSEncryptionKmsKey` and `RDSmonitoringEncryptionKmsKey` (carried over from v6, no change required).
