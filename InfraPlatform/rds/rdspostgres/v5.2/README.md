...
| DBSubnetCidrAZ4 | DB Subnet CIDR for AZ4 (optional) | "10.1.4.0/28" | Optional |
| DBSubnetCidrAZ5 | DB Subnet CIDR for AZ5 (optional) | "10.1.5.0/28" | Optional |
| DBSubnetCidrAZ6 | DB Subnet CIDR for AZ6 (optional) | "10.1.6.0/28" | Optional |
...
| **UseExistingSubnetGroup** (NEW v5.1) | Toggle to reuse an existing subnet group instead of creating a new one. `'true'` for v3/v4/v5 → v5.2 upgrade; `'false'` for greenfield. | "false" | No (default `'false'`) |
...
## Upgrading from v3/v4/v5 to v5.2

### Why this section exists
v5.2 inherited from v5.1: the DB subnet group naming convention from
...
### v3/v4/v5 → v5.2 upgrade (existing deployment with data)
...
New databases created from v5.2 onward should use the default (greenfield) path.
