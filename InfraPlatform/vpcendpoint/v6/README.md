...
| CreateSESEndpoint | <i>String</i> | true, false | Enable SES SMTP endpoint (port 587) | No (defaults to false) |
| CreateSESAPIEndpoint | <i>String</i> | true, false | Enable SES API endpoint (port 443) | No (defaults to false) |
...
| CreateStepFunctionEndpoint | <i>String</i> | true, false | Enable Step Functions endpoint | No (defaults to false) |
| CreateSchedulerEndpoint | <i>String</i> | true, false | Enable EventBridge Scheduler endpoint | No (defaults to false) |
| CreateEventBridgeEndpoint | <i>String</i> | true, false | Enable EventBridge event-bus endpoint | No (defaults to false) |
...
| CreateFirehoseEndpoint | <i>String</i> | true, false | Enable Amazon Kinesis Data Firehose endpoint | No (defaults to false) |
...
| SES SMTP | CreateSESEndpoint | Creates endpoint for Amazon SES SMTP (email-smtp, port 587) |
| SES API | CreateSESAPIEndpoint | Creates endpoint for Amazon SES API (email, port 443) |
...
| Step Functions | CreateStepFunctionEndpoint | Creates endpoint for AWS Step Functions |
| EventBridge Scheduler | CreateSchedulerEndpoint | Creates endpoint for Amazon EventBridge Scheduler (`com.amazonaws.{region}.scheduler`) |
| EventBridge event buses | CreateEventBridgeEndpoint | Creates endpoint for Amazon EventBridge event buses (`com.amazonaws.{region}.events`) |
...
| Firehose | CreateFirehoseEndpoint | Creates endpoint for Amazon Kinesis Data Firehose (`com.amazonaws.{region}.kinesis-firehose`) |

## How to provision template

HIP Team will copy `parameters-vpcendpoint.yaml` to the project's IaC repository. Project team updates the YAML values based on the available [parameters](#parameters-and-its-valid-values) in their own project repository.

YAML is the primary parameter format for this template (self-documenting with `<edit_here>` placeholders and inline comments). The full file lives at `v6/Env/parameters-vpcendpoint.yaml`.

### Example Usage

To enable specific endpoints, set their corresponding parameters to `"true"` in the YAML file. For example:

```yaml
AppShortName: "myapp"
EnvName: "nprd-dev"
AWSRegion: "ap-southeast-1"
VpcId: "vpc-0f7c715e458acd938"
AppSubnetIds: "subnet-aaa,subnet-bbb"
VpcCidr1: "10.193.0.0/16"
VpcCidr2: "10.194.0.0/16"
VpcCidr3: "10.195.0.0/16"

CreateGlueEndpoint: "true"
CreateAthenaEndpoint: "true"
CreateFirehoseEndpoint: "true"
CreateSchedulerEndpoint: "true"
CreateEventBridgeEndpoint: "true"
```
...
9. SES has two separate endpoints: SMTP (port 587, dedicated security group) and API (port 443, main security group)
10. ACM endpoint enables private API calls (`ListCertificates`, `DescribeCertificate`, etc.) from inside the VPC without internet access
11. EventBridge Scheduler uses the main security group with standard HTTPS (443) access
12. The Scheduler endpoint provides private access for workloads calling Scheduler APIs; delivery to scheduled targets is controlled separately by the Scheduler execution role
13. The EventBridge endpoint allows workloads in private subnets to send custom events and call EventBridge APIs without internet access
