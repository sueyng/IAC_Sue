# ELASTIC KUBERNETES SERVICE (EKS) - Version 7
...
## ✅ Current Status
**EKS v7 represents the latest production-ready template with EKS etcd secrets encryption, Access Entry API authentication, multi-project support and enhanced networking**
- ✅ **EKS etcd Secrets Encryption**: Kubernetes secrets encrypted at rest using KMS key by default (new in v7)
...
- ✅ **Consistent Tagging**: All resources tagged with InfraPlatform-EKS-v7

## What's New in Version 7

### 🔒 **EKS etcd Secrets Encryption (NEW)**
- **Envelope Encryption**: Kubernetes secrets stored in etcd are now encrypted at rest using the existing KMS key (`EKSkmsKey`)
- **EncryptionConfig**: Added conditional `EncryptionConfig` block to the EKS cluster resource with `secrets` resource type
- **Default Secure**: `EnableEKSSecretsEncryption` defaults to `true` for new deployments
- **Addresses CxOne Finding**: Resolves Checkmarx high severity finding for undefined `EncryptionConfig` when encryption is enabled

### 🔐 **Security Group Hardening (NEW)**
- **Removed all-protocols rules**: Replaced `IpProtocol: '-1'` (all protocols, ports 0-65535) with specific port rules
- **Minimum required ports**: TCP 443 (HTTPS/API server), TCP 10250 (Kubelet), TCP/UDP 53 (DNS)
- **Applied to**: Control plane → node, node ↔ node, node → control plane communication
- **Source restriction**: All rules restricted by SourceSecurityGroupId — only cluster members can communicate
- **Ref**: [AWS EKS Security Group Requirements](https://docs.aws.amazon.com/eks/latest/userguide/sec-group-reqs.html)
- **Addresses CxOne Finding**: Resolves Checkmarx medium findings for "Security Group Ingress With All Protocols" and "Security Group Ingress With Port Range"

### Note: AWS-Managed Cluster Security Group
> When EKS creates a cluster, AWS automatically provisions an additional cluster security group (`eks-cluster-sg-*`) with allow-all inbound/outbound rules. This is AWS-managed and cannot be controlled via CloudFormation. It is separate from the template-managed security groups (`ClusterSharedNodeSecurityGroup`, `ControlPlane` SG) which are hardened with specific port rules in v7. The AWS-managed SG also includes an EFA (Elastic Fabric Adapter) rule with description "Allows EFA traffic, which is not matched by CIDR rules" — this is standard for all EKS clusters.

### Existing Cluster Upgrade Consideration

> **Important:** EKS secrets encryption should stay enabled for new project deployments. Existing clusters running v6 or earlier must set `EnableEKSSecretsEncryption` to `false` during CloudFormation upgrade if they need to avoid cluster replacement.

- **New Projects**: Deploy v7 template directly with `EnableEKSSecretsEncryption: "true"` — encryption enabled from day one
- **Existing Projects**: Set `EnableEKSSecretsEncryption: "false"` for the CloudFormation upgrade path. To enable encryption on existing clusters, use AWS Console manually (EKS → Select cluster → Overview → Manage encryption) after upgrade planning.
- **Authentication Migration**: For direct migration from v5, use `EKSAuthenticationMode: "API_AND_CONFIG_MAP"` first, then move to `API` after access entries are confirmed.

### Important Considerations
- **Irreversible**: Once etcd encryption is enabled, it **cannot be disabled** — this is an AWS limitation
- **CloudFormation Replacement Risk**: AWS CloudFormation treats `EncryptionConfig` changes on `AWS::EKS::Cluster` as replacement. Do not switch `EnableEKSSecretsEncryption` from `false` to `true` on an existing stack unless cluster replacement is accepted.
- **Verification**: After enabling encryption, verify via AWS Console (EKS → Select cluster → Overview tab → Encryption section)
...
* **IAM Configuration**:
  ...
  - Managed Policies:
    ...
    - S3 Access Policy (for read/write access to application S3 buckets scoped to `AppShortName*`)
...
* **Key Components**:
  ...
  - S3 read/write access (scoped to `AppShortName*` buckets)
  - Three profile-specific SSM parameters publishing the effective Secrets Manager, SQS and S3 managed-policy ARNs
...
| EKSAuthenticationMode | String | API, CONFIG_MAP, API_AND_CONFIG_MAP | API | No | EKS cluster authentication mode. Use API_AND_CONFIG_MAP when migrating from v5 before moving to API |
...
| EnableEKSSecretsEncryption | String | true, false | true | No | Enables EKS secrets encryption via EncryptionConfig. Set false only for existing cluster upgrades to avoid replacement |
...
| FargateProfileName | String | frontend, backend, batch | Yes | Short application profile purpose (maximum 15 characters); do not include AppShortName or EnvName |
...
> **Multiple-profile naming:** Supply only the application profile purpose in `FargateProfileName`, such as `frontend`, `backend` or `batch`. The template automatically uses `{AppShortName}-{EnvName}-{FargateProfileName}` for explicitly named IAM resources, preventing duplicate application and environment text while keeping the names unique.
>
> **Policy ARN parameters:** The template publishes the effective Secrets Manager, SQS and S3 managed-policy ARNs as `{AppShortName}-{EnvName}-{FargateProfileName}-parameter-SecretManagerIamPolicyArn`, `{AppShortName}-{EnvName}-{FargateProfileName}-parameter-SQSIamPolicyArn` and `{AppShortName}-{EnvName}-{FargateProfileName}-parameter-S3IamPolicyArn`.
>
> **Existing-stack impact:** Updating an earlier v7 Fargate-profile stack changes the physical names of its managed policies and Pod service-account role. Changing an existing `FargateProfileName` value to the new purpose-only format also replaces the immutable Fargate profile and can reschedule matching pods. When `SecondProjectForExistingEKS` is `yes`, replacing the Fargate Pod Execution role changes its ARN and also replaces the profile. Review the change set, Kubernetes service-account role annotation and workload rescheduling impact before executing. Production retention settings can leave previous IAM resources for controlled cleanup.
...
2. Azure Pipelines configured with appropriate AWS permissions
...
6. **EKS Authentication Mode**:
   - v6 uses **API mode** (EKS Access Entry API) for cluster authentication
   - The cluster creator (IAM principal deploying the stack) automatically gets admin access
   - For v5 to v6 migration, see the [v5 to v6 Migration Guide](#v5-to-v6-migration-guide) section

7. **Managing EKS Access Entries** (Project Team Responsibility):

   Access entries are **NOT managed by this CloudFormation template**. Project teams should manage access entries via AWS Console to avoid drift issues.
   ...
   **Note**: The cluster creator (IAM principal that deployed the CloudFormation stack) automatically gets admin access.

8. **Cluster Creator Admin Access**: The IAM principal that deploys the CloudFormation stack automatically receives cluster admin access via EKS Access Entry.
...
> **v7 is encrypted by default for new projects.** Existing clusters can use the upgrade path only when `EnableEKSSecretsEncryption` is explicitly set to `false` to avoid CloudFormation replacing the EKS cluster.

### Why Encryption Must Be Disabled During Existing Cluster Upgrade
- Adding `EncryptionConfig` to an existing EKS cluster via CloudFormation triggers a **cluster replacement** (delete and recreate)
- This is a CloudFormation limitation for the `EncryptionConfig` property
- Existing projects upgrading from v5/v6 should set `EnableEKSSecretsEncryption: "false"` and enable encryption manually via AWS Console if needed
- For v5 authentication migration, set `EKSAuthenticationMode: "API_AND_CONFIG_MAP"` first, then move to `API` after access entries are confirmed

### Enabling Encryption on Existing Clusters (Manual)
For existing clusters that need encryption compliance, enable it directly via AWS Console after using `EnableEKSSecretsEncryption: "false"` for the CloudFormation upgrade:

1. Navigate to **AWS Console** → **EKS** → **Clusters** → Select your cluster
2. Go to **Overview** tab → Click **Manage encryption**
3. Select the existing KMS key → Click **Associate**
4. Wait for encryption status to change to `ACTIVE` (takes a few minutes)
5. Verify: Existing secrets will be encrypted on next update; new secrets are encrypted immediately

> **Note**: After manual encryption, do not switch `EnableEKSSecretsEncryption` back to `true` on the same CloudFormation stack unless the team accepts cluster replacement risk.
